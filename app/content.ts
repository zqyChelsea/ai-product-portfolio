...
type UiCopy = {
  ...
  evidenceNote: string; contents: string;
};
...
export const ui: Record<Locale, UiCopy> = {
  'zh-cn': {
    ...
    workIntro: '五个案例，分别回答：如何定义问题、设计 AI 系统、验证效果，并为失败留出安全边界。',
    ...
    evidenceNote: '这些结果来自项目记录或简历。没有统一基线的指标会明确标注为项目观察，而非模型性能结论。',
  },
  'zh-hk': {
    ...
    workIntro: '五個案例，分別回答：如何定義問題、設計 AI 系統、驗證效果，並為失敗留下安全邊界。',
    ...
    evidenceNote: '結果來自項目記錄或履歷；沒有統一基線的數字會明確標示為項目觀察，而非模型性能結論。',
  },
  en: {
    ...
    workIntro: 'Five cases showing how I frame problems, design AI systems, evaluate outcomes and leave room for safe failure.',
    ...
    evidenceNote: 'Results come from project records or the résumé. Metrics without a common baseline are labelled as project observations, not universal model-performance claims.',
  },
  ja: {
    ...
    workIntro: '課題定義、AIシステム設計、成果検証、失敗時の安全設計を示す5つのケースです。',
    ...
    evidenceNote: '結果はプロジェクト記録または履歴書に基づきます。共通ベースラインのない数値はモデル性能ではなく、プロジェクト内の観察として示します。',
  },
};
...
const shared: Record<ProjectSlug, Pick<Project, 'slug'|'no'|'year'|'image'|'role'|'status'|'tools'>> = {
  'academic-compass': { slug:'academic-compass', no:'01', year:'2025—26', image:'/projects/academic-compass.webp', role:'Independent product & build', status:'Prototype · Evaluated', tools:['Dify','RAG','Multi-agent','Python'] },
  'gptutor-rag': { slug:'gptutor-rag', no:'02', year:'2024—26', image:'/projects/gptutor-rag.webp', role:'Core R&D', status:'Piloted in courses', tools:['RAG','Information architecture','Evaluation'] },
  'player-feedback': { slug:'player-feedback', no:'03', year:'2025', image:'/projects/player-feedback.webp', role:'Full-stack game tech', status:'ChinaJoy demo', tools:['OneBot','TypeScript','Node.js','NLP'] },
  'wellness-recommendation': { slug:'wellness-recommendation', no:'04', year:'2024', image:'/projects/wellness-recommendation.webp', role:'AI prompt & front-end', status:'Commercial pilot', tools:['Prompt pipeline','Dataset','Web UX'] },
  'llm-crawler-security': { slug:'llm-crawler-security', no:'05', year:'2025', image:'/projects/llm-crawler-security.webp', role:'NTU undergraduate researcher', status:'Research prototype', tools:['Threat modeling','LLM crawlers','Multi-agent','Evaluation'] },
};
...
const zhHk = zhCn.map((item) => ({...item,
  title:item.title.replaceAll('顾问','顧問').replaceAll('优化','優化').replaceAll('实时','實時').replaceAll('智能台','智能台').replaceAll('推荐','推薦').replaceAll('威胁','威脅'),
...
const ja: Project[] = en.map((item, index) => {
  const titles = ['Agent型アカデミックアドバイザー','GPTutor RAG最適化','リアルタイム・プレイヤーフィードバック','エッセンシャルオイル推薦体験','LLMクローラー脅威研究'];
  const summaries = ['曖昧さを確認し、責任の境界を理解するAIアドバイザー。','一節ではなく、コース全体の構造を理解させる。','自然な会話から、行動可能なプロダクトシグナルを抽出する。','「不調」から「選べる」までの距離を短くする。','ウェブページがAIクローラーを欺くリスクを測定可能にする。'];
  const problems = ['規則は複数の文書に分散し、担当者は多数の学生を支援している。従来のチャットボットは自然言語と曖昧な状況に弱かった。','長い講義資料には表、数式、章をまたぐ関係があり、単純な分割では構造が失われる。','フィードバックがサポート、SNS、チャットに散在し、製品判断まで時間がかかる。','ユーザーは症状で相談する一方、サイトは商品分類で情報を提示していた。','LLMクローラーの敵対的な画像抽出リスクには、分類と専用評価が不足していた。'];
  const solutions = ['責任領域別のマルチAgent、文書取り込み、確認質問、拒否と有人エスカレーションを設計した。','階層的チャンクとメタデータで章構造を保持し、章横断の質問で継続評価した。','QQ／Discord botとダッシュボードを構築し、ルールと軽量NLPで版・機能・感情別に集約した。','ニーズと精油を結ぶデータを整備し、推薦会話を商品ページと使用方法へ接続した。','3種類の攻撃、2種類の防御、2つのクローラー方式をマルチAgent環境で比較した。'];
  const impacts = ['約85–90%の検索精度・単純回答精度、SUS 74.2。30件の実質問で旧システムを上回った。','50件の章横断テストを実施。70–80%の回答者が改善を感じ、試験期の訪問は約60%増加した。','企業パイロットとChinaJoy 2025で検証し、会期中は1日約500インタラクションを記録した。','一般的な推薦依頼の約70%をカバーし、プロジェクト記録では売上が約10%増加した。','精度と再現率を用いた20ラウンドで脅威と防御効果を比較した。'];
  return {...item,title:titles[index],summary:summaries[index],problem:problems[index],solution:solutions[index],impact:impacts[index],context:problems[index]+' 技術だけでなく、ユーザーの判断と安全への影響をプロダクト課題として扱った。',decisions:['ユーザーの負担とリスクから問題を定義する','AIの出力を検証可能な証拠と結ぶ','不確実な場合のフォールバックを設計する'],system:solutions[index]+' 入力、判断、検索／処理、回答、検証の流れを一つのシステムとして設計した。',evaluation:impacts[index]+' 次段階ではデータセット、採点方法、誤り分類をさらに公開する必要がある。',reflection:'AIの能力ではなく、ユーザーがより良い判断をできるかを成功基準にする。'};
});
...
const localMeta: Record<Locale, {roles:string[];statuses:string[]}> = {
  'zh-cn':{roles:['独立产品设计与开发','核心研发','全栈游戏技术','AI 提示与前端','南洋理工本科研究员'],statuses:['原型 · 已评估','课程试点','ChinaJoy 展示','商业试点','研究原型']},
  'zh-hk':{roles:['獨立產品設計與開發','核心研發','全端遊戲技術','AI 提示與前端','南洋理工本科研究員'],statuses:['原型 · 已評估','課程試點','ChinaJoy 展示','商業試點','研究原型']},
  en:{roles:['Independent product & build','Core R&D','Full-stack game tech','AI prompt & front-end','NTU undergraduate researcher'],statuses:['Prototype · Evaluated','Piloted in courses','ChinaJoy demo','Commercial pilot','Research prototype']},
  ja:{roles:['個人プロダクト設計・開発','コアR&D','フルスタック開発','AIプロンプト・フロントエンド','南洋理工大学 学部研究員'],statuses:['プロトタイプ・評価済み','授業パイロット','ChinaJoy展示','商用パイロット','研究プロトタイプ']},
};
