...
export function PortfolioHome({ locale, localeRedirect = false }: { locale: Locale; localeRedirect?: boolean }) {
  ...
  const proofLabels = {
    'zh-cn':['RAG 检索／简单回答','SUS · Academic Compass','跨章节测试问题','对抗性实验轮次'],
    'zh-hk':['RAG 檢索／簡單回答','SUS · Academic Compass','跨章節測試問題','對抗性實驗輪次'],
    en:['RAG retrieval / simple answers','SUS · Academic Compass','cross-chapter test questions','adversarial test rounds'],
    ja:['RAG検索／単純回答','SUS · Academic Compass','章横断テスト質問','敵対的テスト回数'],
  }[locale];
  return (
    <main>
      ...
      <section className="proof-strip" aria-labelledby="proof-title">
        ...
        <div><strong>85–90%</strong><span>{proofLabels[0]}</span></div>
        <div><strong>74.2</strong><span>{proofLabels[1]}</span></div>
        <div><strong>50</strong><span>{proofLabels[2]}</span></div>
        <div><strong>20</strong><span>{proofLabels[3]}</span></div>
      </section>
      ...
      <section className="work-section section-pad" id="work" aria-labelledby="work-title">
        <header className="section-heading work-heading"><span>02 / SELECTED WORK · 01—05</span><div><h2 id="work-title">{copy.selectedWork}</h2><p>{copy.workIntro}</p></div></header>
        <div className="gallery">
          {projects.map((project, index) => (
            <Link className={`project-card project-card-${index + 1}`} key={project.slug} href={localePath(locale, `/projects/${project.slug}/`)} style={{ viewTransitionName: `project-${project.slug}` }}>
